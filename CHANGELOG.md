...
- Added default-off `research.external_synthesis.enabled` config, `grados external-synthesis is-enabled --quiet`, `grados external-synthesis status --json`, and host-agent protocol docs for using GRaDOS's current ChatGPT Pro browser route without making GRaDOS share unverified evidence.
...
- Changed ChatGPT external-synthesis recovery to persist only recoverable `/c/<id>` conversation URLs, keep shell/project URLs as `last_observed_url` diagnostics, preserve `min_turn_index` across recovery, and report `conversation_url_missing_or_not_recoverable` instead of reopening the ChatGPT home page.
...
- Fixed ChatGPT external-synthesis pending recovery so stale or polluted Operation Registry conversation URLs are cleared before `get_operation_status(detail=true)` returns recovery metadata.
