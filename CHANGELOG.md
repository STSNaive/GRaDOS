...
### Added
- Added `extract.headless_browser.disable_pdf_viewer=true` and `extract.headless_browser.download_inbox="browser_inbox"` so GRaDOS-managed browser PDF downloads land in `GRADOS_HOME/browser_inbox/` before DOI/session/hash validation and canonical materialization.
- Added `consult_chatgpt_pro`, the default ChatGPT Pro consult route for enabled external consult. It accepts a required prompt plus optional pack, packet, artifact, or file context, records model/thinking strategy metadata, persists transcript/snapshot/capture artifacts, supports `manual_response` pasted fallback saves, and uses bounded auto-reattach without resending the prompt.
...
- Added `get_operation_status`, a generic MCP status/recovery tool for pending external consult sessions, DOI-bound PDF parse attempts, indepth search runs, local PDF import runs, and Codex download handoffs.
...
- Added `run_external_consult`, a packet-context route for enabled external consult. It prepares or verifies evidence packets, uses a private GRaDOS ChatGPT profile, captures the advisory response, and leaves explicit audit/recovery tools available for canonical reread.
- Added ChatGPT browser-mode runtime scaffolding for external consult, including private profile/session paths, first-time `grados external-consult setup-browser`, `grados external-consult doctor [--live]`, profile readiness checks, GRaDOS ChatGPT model/thinking selector helpers, response capture, and operational session records.
...
- Added `prepare_external_consult_from_topic`, a higher-level MCP route that prepares an evidence pack from a topic and persists a verified external-consult packet while keeping the lower-level pack/packet tools available for recovery and explicit control.
...
- Added deterministic external consult packet/result tools: `preview_external_consult_packet`, `prepare_external_consult_packet`, `save_external_consult_result`, and `audit_external_consult_result`, keeping ChatGPT Pro advisory output tied to current-valid evidence packs before canonical rereading.
- Added default-off `research.external_consult.enabled` config, `research.external_consult.response_wait_total_seconds`, `grados external-consult is-enabled --quiet`, `grados external-consult status --json`, and host-agent protocol docs for using GRaDOS's current ChatGPT Pro browser route without making GRaDOS share unverified evidence.
...
### Changed
- Changed browser PDF capture telemetry to include capture hashes, DOI/session binding, browser-inbox download diagnostics, attribution-ambiguous markers after manual attention, ScienceDirect challenge controlled-wait events, and richer PDF materialization conflict receipts.
- Changed the ChatGPT Pro external surface from external synthesis to external consult while expanding the Pro mode around `consult_chatgpt_pro`: `research.external_synthesis.enabled` -> `research.external_consult.enabled`, `grados external-synthesis ...` -> `grados external-consult ...`, and `external_synthesis_*` tools/artifacts -> `external_consult_*` names. No legacy config or CLI aliases are retained in this release.
- Changed the default MCP research profile to expose `consult_chatgpt_pro` instead of the topic-or-pack `run_external_consult` packet route; `run_external_consult` remains available in external recovery toolsets and no longer auto-audits by default.
- Changed ChatGPT external-consult recovery so `get_operation_status(detail=true)` performs one short no-resend recovery probe that saves an already available assistant response or returns pending with the same recovery handle, while `detail=false` remains read-only.
...
- Changed the release script and PyPI publish workflow to block a higher release tag when the previous existing release tag has not reached PyPI, publish the same `dist/` artifacts that passed release verification, serialize PyPI publish runs, and verify the exact tag version through bounded post-publish `uv tool install` / `uvx` retries instead of treating PyPI JSON visibility as the success gate.
- Changed release automation to create annotated version-only tags locally while deferring GitHub Release creation until after PyPI install verification; existing remote tags dispatch a recovery mode that skips source revalidation and PyPI upload, then restores PyPI install verification plus GitHub Release creation/update. GitHub Releases remain titled `vX.Y.Z`, with release guard and release notes generated by current release tooling instead of historical tag copies.
...
- Changed `run_external_consult` to persist session/packet/prompt recovery state in the Operation Registry before waiting and to return a pending operation receipt before host tool timeouts; status recovery can capture/save/audit the final response without resending the prompt.
- Changed ChatGPT external-consult recovery to persist only recoverable `/c/<id>` conversation URLs, keep shell/project URLs as `last_observed_url` diagnostics, preserve `min_turn_index` across recovery, and report `conversation_url_missing_or_not_recoverable` instead of reopening the ChatGPT home page.
...
- Changed external consult from a host/manual ChatGPT handoff into the default GRaDOS-native browser workflow behind the `research.external_consult.enabled` switch, while keeping lower-level packet/save/audit tools for recovery and leaving the separate `extract.fetch_strategy.codex` PDF acquisition route intact.
...
- Changed ChatGPT external-consult setup to close the setup browser after login by default, keep `--keep-open` for debugging, and share the private profile lock across the default setup flow, live doctor checks, and consult runs.
- Changed the publisher `browser` fetch strategy to use GRaDOS browser runtime stability and recovery controls while keeping the existing acquisition contract: browser code captures PDF bytes or returns manual-resume challenge metadata, and only the downstream parser/QA/persist pipeline writes canonical `papers/*.md`.
...
- Changed `save_external_consult_result` to default to `audit=true`, returning the required external-consult audit result immediately after saving unless explicitly disabled for recovery/debug workflows.
...
- Changed external consult audit to validate ChatGPT Pro references against the saved packet when one is linked, accept structured `claims[].anchor_ids` without requiring author-year prose citations, and store packet artifacts without embedding a duplicate full host prompt.
...
- Changed browser challenge handling to propagate `manual`, `host`, `resume`, and `fetch_trace` metadata through `fetch_paper`, extraction receipts, and `remote_metadata`, while keeping acquisition diagnostics out of canonical `papers/*.md` frontmatter and embedding text; `extract_paper_full_text(resume_browser=true)` now starts at the browser strategy with the saved URL/profile after publisher verification instead of rerunning the full `api`-first chain.
...
- Fixed retained publisher-browser cleanup so completed PDF captures release the profile lock, retained challenge pages keep it only while user action is needed, and unattributed browser download captures are marked as possibly assisted.
- Fixed ChatGPT external-consult live doctor/recovery mismatch by checking the no-submit consult route through model, thinking, and composer readiness, and by treating pre-submit `chatgpt_login_required` failures without a recoverable handle as failed rather than pending.
- Fixed ChatGPT external-consult pending recovery so stale or polluted Operation Registry conversation URLs are cleared before `get_operation_status(detail=true)` returns recovery metadata.
- Fixed draft audit author-year citation handling so narrative forms such as `Dou et al. (2026)`, `Dou et al., 2026`, and `张三等，2025` count as citation markers without leaking author/year text into retrieval queries.
...
- Fixed `grados external-consult setup-browser --keep-open` so the ChatGPT private profile lock is held until the kept-open setup browser closes, preventing live doctor/consult runs from racing the same browser profile.
...
- Added regression coverage for idempotent extraction reuse with `force_refresh`, uncapped `indepth` materialization across more than eight candidates, topic-to-packet external consult preparation, default save-and-audit behavior, and optional pack-audit suggestions.
