...
- Changed the `scihub` fetch strategy to honor `extract.sci_hub.auto_update_mirror` and `mirror_url_file`: when enabled, it resolves Wikipedia infobox Sci-Hub domains before local cache and static `fallback_mirror` candidates.
