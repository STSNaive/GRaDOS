...
- Changed ChatGPT external-consult recovery so `get_operation_status(detail=true)` performs bounded reattach/capture attempts, while `detail=false` remains read-only.
