...
如果你希望一次性接入最常见的完整研究工作流，可以把这三个 MCP 服务一起配置。下面这个例子假设配置文件在 `D:/Projects/Papers/grados-config.json`，`extract.papersDirectory` 使用默认相对路径 `./markdown`，并且 `localRag.dbPath` / `localRag.cacheDir` 保持默认相对路径 `./lancedb` / `./models`。在这组默认值下，`mcp-local-rag` 对应的路径如下。
...
| **项目文件** | 配置文件所在目录（`PROJECT_ROOT`） | `downloads/`、`markdown/`、`scihub-mirrors.txt` |
