...
5. **External synthesis:** only after the CLI gate passes, use the route in [references/external_synthesis.md](references/external_synthesis.md). Prefer `grados:run_external_synthesis`; it prepares or verifies the pack, uses the private ChatGPT browser profile, saves the response, and audits by default. Use `grados:preview_external_synthesis_packet`, `grados:prepare_external_synthesis_from_topic`, `grados:prepare_external_synthesis_packet`, `grados:save_external_synthesis_result`, and `grados:audit_external_synthesis_result` only for dry runs, recovery, or explicit reruns.
