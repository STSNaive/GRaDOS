...
Before using external consult, run the GRaDOS CLI gate with the same `GRADOS_HOME` as the active server: `grados external-consult is-enabled --quiet` (or `uvx grados external-consult is-enabled --quiet` when using the plugin launcher). Use external consult only when the command exits with code 0; if the command is unavailable, fails, or exits nonzero, ignore [references/external_consult.md](references/external_consult.md) and do not use external consult. First-time ChatGPT login for this route is handled by `grados external-consult setup-browser`.
...
2. **User provides local PDFs:** use `grados:import_local_pdf_library` for a directory or `grados:parse_pdf_file` for one PDF. If either returns pending, poll `grados:get_operation_status`; after completion, use `grados:get_saved_paper_structure` and `grados:read_saved_paper`.
...
5. **External consult:** only after the CLI gate passes, use the route in [references/external_consult.md](references/external_consult.md). Prefer `grados:consult_chatgpt_pro`; it requires a prompt, accepts optional pack/packet/artifact/file context, uses the private ChatGPT browser profile, saves advisory output, and does not auto-audit. If it returns pending, call `grados:get_operation_status(detail=true)` instead of resending the prompt. Use `grados:preview_external_consult_packet`, `grados:prepare_external_consult_from_topic`, `grados:prepare_external_consult_packet`, `grados:save_external_consult_result`, `grados:audit_external_consult_result`, and `grados:run_external_consult` only for dry runs, recovery, explicit audit, or topic/pack packet workflows.

For ordinary research, do not start with audit, comparison, external consult, Zotero, failure memory, or generic artifact tools unless the user specifically asks for that mode or the task state requires recovery. Do not manually orchestrate fetch strategy, parser fallback, canonical save, indexing, remote-metadata updates, pack persistence, pack verification, optional dry runs, or snapshot inspection when a higher-level GRaDOS tool already performs that step.
...
3. `grados:extract_paper_full_text`, `grados:import_local_pdf_library`, `grados:parse_pdf_file`, and `grados:get_operation_status` are storage/indexing/recovery actions. Their receipts are not citable.
...
6. Use `search_academic_papers(indepth=true)` when the user asks for breadth, checkpointing, or immediate materialization of returned candidates. `indepth` uses the same search `limit`, returns a pending operation id, and still produces navigation material, not final citation evidence.
...
3. If extraction, parsing, indepth search, local import, or external consult returns `status=pending` / `parse_in_progress`, poll `grados:get_operation_status` and do not repeat the same long-running request unless the status says to retry.
4. For many candidates, extract in manageable batches and continue while new papers are likely to improve coverage, reveal useful references, or clarify uncertainty.
5. If extraction fails for a strongly relevant paper, record its title, DOI, and abstract-based relevance in `未能获取全文`; silently skip marginal failures.
