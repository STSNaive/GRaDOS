...
| `grados:get_operation_status` | Inspect durable pending operations from external synthesis, DOI-bound PDF parse attempts, indepth search, and local PDF import. With `detail=true`, recover/capture a ChatGPT browser session without resending the original prompt. |
