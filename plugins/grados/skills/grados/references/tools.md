...
| `grados:get_operation_status` | Inspect durable operations from external consult, DOI-bound PDF parse attempts, indepth search, local PDF import, and Codex download handoff. With `detail=true`, recover/capture a ChatGPT browser session without resending the original prompt and return registry events/debug pointers. ChatGPT detail recovery only reopens saved recoverable `/c/<id>` URLs; missing or shell URLs report `conversation_url_missing_or_not_recoverable`. |
...
| `grados:get_operation_status` | `operation_id` minLength=1; `detail` defaults to false. |
