...
| GRaDOS | `extract_paper_full_text` | Fetch, parse, QA-check, and save one paper's canonical full text by DOI. If the DOI is already saved, default `force_refresh=false` returns an already-saved receipt; set `force_refresh=true` to refetch/reparse. |
