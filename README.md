...
| GRaDOS | `get_operation_status` | Inspect pending external synthesis, DOI-bound PDF parse, indepth search, or local PDF import operations. With `detail=true`, recover a ChatGPT browser response without resending the prompt. |
| GRaDOS | `ingest_codex_downloaded_pdf` | Complete a `codex` Chrome-extension handoff by validating either `downloaded_file_path` or one scanned watch-dir candidate, then reuse the same canonical parse/save path. Ambiguous, missing, or invalid candidates are recorded as recoverable failures; long parser runs return in-progress rather than parse failure. |
