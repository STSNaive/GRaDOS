...
| External synthesis | Gate first, then use `grados:run_external_synthesis` with either a topic or a current-valid pack id; if it returns pending, poll `grados:get_operation_status(detail=true)`. | The default route prepares or verifies the pack, persists the packet/session/prompt hash before waiting, uses GRaDOS's private ChatGPT browser profile, confirms GRaDOS-validated Pro model and Pro Extended thinking route, captures the response, saves it, and audits it. Packets with missing scoped DOI coverage or non-evidence anchors are not sendable. Long generations return recoverable operation receipts. Keep lower-level preview, packet, save, and audit tools for dry runs, recovery, and explicit reruns. |
