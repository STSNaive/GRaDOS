...
| Evidence organization | Saved papers -> `grados:build_evidence_grid`, `grados:compare_papers`, or `grados:get_papers_full_context(mode="estimate")` -> accepted anchors reread with `grados:read_saved_paper`. | Helper tools rank, align, or budget context, but they do not create final citation evidence. Scoped evidence grids/packs report requested, covered, and missing DOI coverage; evidence packs filter References/backmatter, title-only, and citation-only fragments before counting coverage. |
...
| External synthesis | Gate first, then use `grados:run_external_synthesis` with either a topic or a current-valid pack id; if it returns pending, poll `grados:get_operation_status(detail=true)`. | The default route prepares or verifies the pack, persists the packet/session/prompt hash before waiting, uses GRaDOS's private ChatGPT browser profile, confirms GRaDOS-validated Pro model and Pro Extended thinking route, captures the response, saves it, and audits it. Packets with missing scoped DOI coverage or non-evidence anchors are not sendable. Long generations return operation receipts; status recovery only reopens a saved recoverable ChatGPT `/c/<id>` URL, while home/project shell URLs stay diagnostic and report `conversation_url_missing_or_not_recoverable`. Keep lower-level preview, packet, save, and audit tools for dry runs, recovery, and explicit reruns. |
...
| `grados:get_operation_status` | Inspect durable operations from external synthesis, DOI-bound PDF parse attempts, indepth search, local PDF import, and Codex download handoff. With `detail=true`, recover/capture a ChatGPT browser session without resending the original prompt and return registry events/debug pointers. ChatGPT detail recovery only reopens saved recoverable `/c/<id>` URLs; missing or shell URLs report `conversation_url_missing_or_not_recoverable`. |
...
| `grados:preview_external_synthesis_packet` | Dry-run a compact external-synthesis packet from one current-valid evidence pack. It reports sendability, blocked reasons, size estimates, prompt hash, and host guidance without saving artifacts or contacting any external service. Optional before prepare, not required for the send path. |
| `grados:prepare_external_synthesis_packet` | Persist an `external_synthesis_packet` artifact with verified anchor ids, canonical paragraph coordinates, short excerpts, candidate claims, limitations, and prompt hash, returning the host prompt as a regenerable view. It refuses stale, non-current, missing-coverage, or non-evidence packets. |
...
| `grados:run_external_synthesis` | Run the default GRaDOS-native ChatGPT Pro browser route. It accepts either a topic or pack id, prepares or verifies the packet, persists session/packet/prompt-hash recovery state, uses the private ChatGPT profile, confirms GRaDOS-validated Pro model and Pro Extended thinking route before sending, captures the advisory response, saves it, audits it, and returns the canonical reread next action. If generation exceeds the foreground wait, it returns `status=pending` with `operation_id`/`browser_session_id`; poll `get_operation_status(detail=true)`. It records only recoverable `/c/<id>` URLs as recovery handles and keeps home/project shell URLs diagnostic. `recover_session_id` remains a compatibility path. |
...
External synthesis packets are a browser-mediated advisory layer on top of current-valid evidence packs. `run_external_synthesis` is the default enabled route: it prepares or verifies the packet, uses GRaDOS's private ChatGPT browser profile, confirms GRaDOS-validated Pro model and Pro Extended thinking route before sending, saves the returned advisory text, and audits it. If packet QA finds missing scoped DOI coverage or non-evidence anchors, the packet is returned with `sendable=false` and is not saved for sending. If ChatGPT is still generating after the foreground wait, the tool returns `status=pending` with `operation_id` / `browser_session_id`; poll `grados:get_operation_status(detail=true)` to capture the same session without resending. Recovery only uses saved recoverable ChatGPT `/c/<id>` URLs; home/project shell URLs stay diagnostics and do not trigger a home-page reopen loop. `recover_session_id` remains a compatibility path. `preview_external_synthesis_packet` is non-mutating; `prepare_external_synthesis_from_topic` and `prepare_external_synthesis_packet` persist `research_artifacts(kind="external_synthesis_packet")` for lower-level recovery; `save_external_synthesis_result(audit=true)` and `audit_external_synthesis_result` remain available for explicit reruns before accepted claims are reread through canonical GRaDOS tools.
